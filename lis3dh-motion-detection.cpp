...
imu_status_t LIS3DH::readRegisterRegion(uint8_t *outputPointer, uint8_t offset, uint8_t length)
{
    ...
    setI2CReading(true);
    WireBeginTransmission(I2CAddress);
    ...
    else  //OK, all worked, keep going
    {
        ...
        WireRequestFrom(I2CAddress, length);
        while ( (WireAvailable()) && (i < length))  // slave may send less than requested
        {
            c = WireRead(); // receive a byte as character
            *outputPointer = c;
            outputPointer++;
            i++;
        }
...
imu_status_t LIS3DH::readRegister(uint8_t* outputPointer, uint8_t offset) {
    ...
    WireRequestFrom(I2CAddress, numBytes);
    while ( WireAvailable() ) // slave may send less than requested
    {
        result = WireRead(); // receive a byte as a proper uint8_t
    }
